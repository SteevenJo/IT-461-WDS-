...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_query($conn, $insertOrderDetails)) {
        echo "<script>alert('Order placed successfully!'); window.location.href = 'search_orders.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error: " . mysqli_error($conn) . "'); window.location.href = 'search_orders.php';</script>";
        exit();
    }
}
